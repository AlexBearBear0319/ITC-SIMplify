...
export default function Header() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      supabase
        .from("profiles")
        .select("username, avatar_url, points_balance, is_admin")
        .eq("id", user.id)
        .single()
        .then(({ data }) => {
        ...
        >
          ...
          <div className="hidden sm:flex flex-col items-start leading-none">
            ...
            <span className="text-[10px] text-gold font-medium mt-0.5">
              ✦ {profile ? profile.points_balance.toLocaleString() : "—"} pts
            </span>
          </div>
          <ChevronDown
            size={14}
            ...
            <div className="px-4 py-2 border-b border-border mb-1">
              ...
              <div className="flex items-center gap-1 mt-0.5">
                ...
                <span className="text-xs text-gold font-medium">
                  {profile ? profile.points_balance.toLocaleString() : "—"} points
                </span>
              </div>
            </div>
