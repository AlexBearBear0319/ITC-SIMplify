...
const MOCK_LOCATIONS_LIST = [
  { id: 1, name: "Main IT Lab"     },
  { id: 2, name: "Library Level 3" },
  { id: 3, name: "Study Corner A"  },
  { id: 4, name: "Canteen Terrace" },
  { id: 5, name: "PC Lab 2"        },
  { id: 6, name: "Reading Room"    },
];
...
const INITIAL_GROUPS: StudyGroup[] = [
  {
    ...
    description: "Working through LeetCode problems and discussing time & space complexity. All levels welcome.",
    max_members: 5,
    current_members: 3,
    is_active: true,
    created_at: "2026-03-17T08:00:00Z",
    profiles:  { username: "sarah_l",  avatar_url: null },
    locations: { name: "Main IT Lab",  category: "IT Lab" },
  },
  {
    id: 2,
    host_id: "uuid-2",
    ...
    description: "Preparing for the mid-term. Covering probability, regression, and hypothesis testing.",
    max_members: 4,
    current_members: 4,
    is_active: true,
    created_at: "2026-03-17T08:15:00Z",
    profiles:  { username: "marcus_t",       avatar_url: null },
    locations: { name: "Library Level 3",    category: "Library" },
  },
  {
    id: 3,
    host_id: "uuid-3",
    ...
    description: "Building a full-stack app with Next.js and Supabase. Pair-coding sessions welcome.",
    max_members: 4,
    current_members: 2,
    is_active: true,
    created_at: "2026-03-17T08:30:00Z",
    profiles:  { username: "priya_k", avatar_url: null },
    locations: { name: "PC Lab 2",    category: "IT Lab" },
  },
  {
    id: 4,
    host_id: "uuid-4",
    ...
    description: "Pandas, NumPy, and Matplotlib practice. Bring your own dataset — we'll figure it out together!",
    max_members: 6,
    current_members: 2,
    is_active: true,
    created_at: "2026-03-17T08:45:00Z",
    profiles:  { username: "alex_w",      avatar_url: null },
    locations: { name: "Study Corner A",  category: "Study" },
  },
  {
    id: 5,
    host_id: "uuid-5",
    ...
    description: "Peer-editing essays and dissertations. Constructive feedback only, all disciplines welcome.",
    max_members: 5,
    current_members: 4,
    is_active: true,
    created_at: "2026-03-17T09:00:00Z",
    profiles:  { username: "li_m",       avatar_url: null },
    locations: { name: "Reading Room",   category: "Library" },
  },
  {
    id: 6,
    host_id: "uuid-6",
    ...
    description: "Going through the CEH syllabus, solving CTF challenges, and network scanning labs.",
    max_members: 3,
    current_members: 1,
    is_active: true,
    created_at: "2026-03-17T09:15:00Z",
    profiles:  { username: "raj_s",    avatar_url: null },
    locations: { name: "Main IT Lab",  category: "IT Lab" },
  },
  {
    id: 7,
    host_id: "uuid-7",
    location_id: 4,
    subject: "Graphic Design & Branding",
    description: "Working on Figma layouts and brand identity projects together. Adobe CC also welcome.",
    max_members: 4,
    current_members: 3,
    is_active: true,
    created_at: "2026-03-17T09:30:00Z",
    profiles:  { username: "nina_c",         avatar_url: null },
    locations: { name: "Canteen Terrace",    category: "Chill" },
  },
];
...
export default function FinderPage() {
  ...
  const handleJoinGroup = (id: number) => {
    if (activeGroupId !== null) return; // one active group at a time
    const group = groups.find((g) => g.id === id);
    if (!group || group.current_members >= group.max_members) return;

    // TODO: replace with Supabase:
    //   await supabase.from("study_group_members").insert({ group_id: id, user_id: session.user.id })
    //   await supabase.from("study_groups").update({ current_members: group.current_members + 1 }).eq("id", id)
    setActiveGroupId(id);
    setGroups((prev) =>
  ...
  const handleLeaveGroup = (id: number) => {
    ...
    const isHost       = currentUser?.id === group.host_id;
    const isLastMember = group.current_members <= 1;

    if (isLastMember) {
      // Last person leaving — deactivate the group
      // TODO: await supabase.from("study_groups").update({ is_active: false }).eq("id", id)
      // TODO: await supabase.from("study_group_members").delete().match({ group_id: id, user_id: currentUser?.id })
      setGroups((prev) => prev.map((g) => g.id === id ? { ...g, is_active: false } : g));
    } else if (isHost) {
      // Host leaving but others remain — transfer host to oldest remaining member
      // TODO:
      //   const { data: oldest } = await supabase
      //     .from("study_group_members")
      //     .select("user_id, joined_at, profiles(username, avatar_url)")
      //     .eq("group_id", id).neq("user_id", currentUser?.id)
      //     .order("joined_at", { ascending: true }).limit(1).single()
      //   await supabase.from("study_groups")
      //     .update({ host_id: oldest.user_id, current_members: group.current_members - 1 })
      //     .eq("id", id)
      //   await supabase.from("study_group_members").delete().match({ group_id: id, user_id: currentUser?.id })
      setGroups((prev) =>
        prev.map((g) => g.id === id ? { ...g, current_members: Math.max(0, g.current_members - 1) } : g)
      );
    } else {
      // Regular member leaving
      // TODO: await supabase.from("study_group_members").delete().match({ group_id: id, user_id: currentUser?.id })
      // TODO: await supabase.from("study_groups").update({ current_members: group.current_members - 1 }).eq("id", id)
      setGroups((prev) =>
        prev.map((g) => g.id === id ? { ...g, current_members: Math.max(0, g.current_members - 1) } : g)
      );
    }
  ...
  const handleCreate = async (form: CreateForm) => {
    ...
    const loc = MOCK_LOCATIONS_LIST.find((l) => l.id === form.location_id);
    const newGroup: StudyGroup = {
      id: Date.now(),
      host_id: currentUser?.id ?? "anonymous",
      ...
      profiles:  { username: currentUser?.username ?? "you", avatar_url: currentUser?.avatar_url ?? null },
      locations: { name: loc?.name ?? "Unknown", category: "Study" },
    };
